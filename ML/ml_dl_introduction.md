...
3차원 사진 데이터를 1차원으로 평면화시켜야 한다면(Fully Connected), 사진 데이터를 평면화 시키는 과정에서 공간 정보가 손실될 수밖에 없다.

결과적으로 이미지 공간 정보 유실로 인한 정보 부족으로 인공 신경망이 특징을 추출 및 학습이 비효율적이고 정확도를 높이는데 한계가 있다.

CNN은 이미지의 공간 정보를 유지한 상태로 학습이 가능한 모델이다.
...
Training data set에만 최적화 됨.

overfitting 되었는지 알려면, 실제 test set을 통해 테스트를 해봤을 때 알 수 있는걸까?
=> training accuracy와 test accuracy를 비교해서 알 수 있다.
...
- Data augmentation
- Early stopping: 현재 이 방법을 많이 쓰고 있음.
- Ensemble Methods: 여러 모델을 합쳐서 학습. 각 모델의 장점만 뽑아낸다.
